<?php
	if($actionsRequired){
		require_once "../models/bibliotecaModel.php";
	}else{ 
		require_once "./models/bibliotecaModel.php";
	}

	class bibliotecaController extends bibliotecaModel{

		/*----------  Add biblioteca Controller  ----------*/
		public function add_biblioteca_controller(){
			...
			$query1=self::execute_single_query("SELECT Titulo FROM biblioteca WHERE Titulo='$title'");

			if($query1->rowCount()>=1){
				$dataAlert=[
					"title"=>"¡Ocurrió un error inesperado!",
					"text"=>"El Nombre del Libro ya se encuentra registrado por favor elija otro",
					"type"=>"error"
				];
				return self::sweet_alert_single($dataAlert);
			}else{
				...
				if(self::add_biblioteca_model($data)){
					$dataAlert=[
						"title"=>"¡Libro registrada!",
						"text"=>"El libro se registró con éxito en el sistema",
						"type"=>"success"
					];
					...
					$dataAlert=[
						"title"=>"¡Ocurrió un error inesperado!",
						"text"=>"No hemos podido registrar el libro, por favor intente nuevamente",
						"type"=>"error"
					];
					return self::sweet_alert_single($dataAlert);
				}
		...
		public function pagination_biblioteca_controller($Pagina,$Registros){
			...
			$Datos=self::execute_single_query("
				SELECT * FROM biblioteca ORDER BY Titulo DESC LIMIT $Inicio,$Registros
			");
			$Datos=$Datos->fetchAll();

			$Total=self::execute_single_query("SELECT * FROM biblioteca");
			...
			if($Total>=1){
				...
				foreach($Datos as $rows){
					...
					<tr>
						...
						<td>'.$rows['Tutor'].'</td>
						<td>
							<a href="'.SERVERURL.'bibliotecainfo/'.$rows['idbiblioteca'].'/" class="btn btn-success btn-raised btn-xs">
								<i class="zmdi zmdi-refresh"></i>
							</a>
						</td>
						<td>
							<form action="'.SERVERURL.'ajax/ajaxBiblioteca.php" method="POST" enctype="multipart/form-data" autocomplete="off" data-form="DellVideo" class="ajaxDataForm">
								<input type="hidden" name="bibliotecaCode" value="'.$rows['idbiblioteca'].'">
			...
			if($Total>=1){
				...
				if($Pagina==1){
					...
					$table.='<li><a href="'.SERVERURL.'bibliotecalist/'.($Pagina-1).'/">«</a></li>';
				}

				for($i=1; $i <= $Npaginas; $i++){
					if($Pagina == $i){
						$table.='<li class="active"><a href="'.SERVERURL.'bibliotecalist/'.$i.'/">'.$i.'</a></li>';
					}else{
						$table.='<li><a href="'.SERVERURL.'bibliotecalist/'.$i.'/">'.$i.'</a></li>';
					}
				}
		...
		public function pagination_bibliotecadescarga_controller($Pagina,$Registros){
			...
			if($Total>=1){
				...
				foreach($Datos as $rows){
					...
					<tr>
						...
						<td>'.$rows['Tutor'].'</td>
						<td>
							<a href="'.SERVERURL.'bibliotecaview/'.$rows['idbiblioteca'].'/" class="btn btn-success btn-raised btn-xs">
								<i class="zmdi zmdi-refresh"></i>
							</a>
						</td>
			...
			if($Total>=1){
				...
				if($Pagina==1){
					...
					$table.='<li><a href="'.SERVERURL.'bibliotecadescarga/'.($Pagina-1).'/">«</a></li>';
				}

				for($i=1; $i <= $Npaginas; $i++){
					if($Pagina == $i){
						$table.='<li class="active"><a href="'.SERVERURL.'bibliotecadescarga/'.$i.'/">'.$i.'</a></li>';
					}else{
						$table.='<li><a href="'.SERVERURL.'bibliotecadescarga/'.$i.'/">'.$i.'</a></li>';
					}
				}
		...
		public function delete_biblioteca_controller($code){
			$code=self::clean_string($code);
			
			$query1=self::execute_single_query("SELECT Adjuntos FROM biblioteca WHERE id='$code'");
			$rows=$query1->fetch();

			//$query2=self::execute_single_query("SELECT Adjunto FROM comentarios WHERE id='$code'");
			//$rows2=$query2->fetch();

			if(self::delete_biblioteca_model($code)){
		...
		public function delete_biblioteca_attachment_controller($code,$name,$urls){
			...
			if(unlink($AttDir.$name)){
				$query1=self::execute_single_query("SELECT * FROM biblioteca WHERE idbiblioteca='$code'");
				$rows=$query1->fetch();
				$filesA=explode(",", $rows['Adjuntos']);
				$catt=0;
				$attFinal="";
				...
				if(self::execute_single_query("UPDATE biblioteca SET Adjuntos='$attFinal' WHERE idbiblioteca='$code'")){
					$dataAlert=[
		...
		public function data_biblioteca_controller($Type,$Code){
			...
			if($bibliotecadata=self::data_biblioteca_model($data)){
				return $bibliotecadata;
			}else{
				$dataAlert=[
					"title"=>"¡Ocurrió un error inesperado!",
					"text"=>"No hemos podido seleccionar los datos del libro",
					"type"=>"error"
				];
				return self::sweet_alert_single($dataAlert);
			}
		...
		public function update_biblioteca_controller(){
			...
			$query1=self::execute_single_query("SELECT Adjuntos FROM biblioteca WHERE idbiblioteca='$code'");
			$rows1=$query1->fetch();
			...
			if(self::update_biblioteca_model($data)){
				$dataAlert=[
