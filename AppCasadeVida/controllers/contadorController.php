<?php
	if($actionsRequired){
		require_once "../models/contadoresDashModel.php";
	}else{ 
		require_once "./models/contadoresDashModel.php";
	}

	class contadorDashModel extends contadoresModel{

		/*----------  Add evaluacion Controller  ----------*/

        public function contadores($action){
            $data = array();
            if ($action=="contar_casa_de_vida"){
                $query=self::connect()->prepare("SELECT count (*) FROM casadevida");
                $query->execute();
			    return $query;
